...
const formatReportRow = C.task({
    ...
    run: (env, input) => ({
        ...
        detail: input.detailText ??
            C.run(env.diagnostics, input.detailDiagnostics ?? []),
    }),
});
const captureTasks = [formatMoney, formatMoneyFr, formatDiagnostics, formatReportRow];
...
function describeCaptureTasks() {
    return captureTasks.map((task) => ({
        id: task.id,
        description: formatCaptureTaskDescription(C.describe(task)),
        envKeys: C.describe(task).envKeys,
