...
function assertValidDeusPath(path, label) {
    ...
    path.forEach((segment, index) => {
        if (typeof segment !== "string" || segment.length === 0 || segment.trim().length === 0) {
            throw new DeusMachinaError("InvalidDeusPath", `${label} segment ${index} must be a non-empty string`);
        }
    });
}
export function formatDeusPath(path) {
    assertValidDeusPath(path, "path");
    return path.join("/");
}
export function parseDeusPath(path) {
...
export function sameDeusPath(a, b) {
    assertValidDeusPath(a, "left path");
    assertValidDeusPath(b, "right path");
    return a.length === b.length && a.every((v, i) => v === b[i]);
}
export function isDeusAncestorPath(ancestor, path) {
...
function normalizeDeusPathInput(path, label) {
    if (typeof path === "string")
        return parseDeusPath(path);
    assertValidDeusPath(path, label);
    return copyPath(path);
}
function resolveTransitionTargetPath(target, ownerPath, label) {
    if (typeof target === "string") {
        if (target.startsWith("/"))
            return parseDeusPath(target);
...
export function defineDeusMachine(machine) {
    ...
    const transitions = machine.transitions.map((t) => {
        ...
        if (!stateKeys.has(pathKey(t.from)))
            throw new DeusMachinaError("UnknownDeusStatePath", `transition ${t.key} from path must exist`);
        if (Array.isArray(t.to) || typeof t.to === "string") {
            ...
            if (!stateKeys.has(pathKey(resolved)))
                throw new DeusMachinaError("UnknownDeusStatePath", `transition ${t.key} to path must exist`);
        }
        else if (t.to !== undefined && typeof t.to !== "function") {
            throw new DeusMachinaError("InvalidDeusTransition", `transition ${t.key} to must be a path or function`);
        }
        ...
        if (t.hysteresis !== undefined) {
            if (typeof t.hysteresis.previous !== "function")
                throw new DeusMachinaError("InvalidHysteresis", `transition ${t.key} hysteresis.previous must be a function`);
            finite(t.hysteresis.margin, "InvalidHysteresis", `transition ${t.key} hysteresis margin`);
            if (t.hysteresis.margin < 0)
                throw new DeusMachinaError("InvalidHysteresis", `transition ${t.key} hysteresis margin must be >= 0`);
        }
        const utilityKeys = new Set();
        for (const u of t.utility ?? []) {
            if (typeof u.key !== "string" || u.key.length === 0 || u.key.trim().length === 0)
                throw new DeusMachinaError("InvalidDeusTransition", `transition ${t.key} utility key must be non-empty`);
            ...
            if (typeof u.score !== "number" && typeof u.score !== "function")
                throw new DeusMachinaError("InvalidUtilityScore", `utility score for ${u.key} must be a number or function`);
...
export function hydrateDeusSnapshot(machine, options) {
...
function resolveSnapshotCreation(machine, boardOrOptions) {
    const hasOptionsShape = !!boardOrOptions &&
        typeof boardOrOptions === "object" &&
        ("statePath" in boardOrOptions ||
            "runEnter" in boardOrOptions);
    if (!hasOptionsShape) {
        return {
...
export function createDeusSnapshot(machine, board) {
    const resolved = resolveSnapshotCreation(machine, board);
...
export function stepDeusMachine(machine, snapshot, event) {
    ...
    const candidates = orderedFrom.flatMap((from) => machine.transitions.map((t) => ({ t })).filter(({ t }) => sameDeusPath(t.from, from)));
    const traces = [];
    let selected;
    candidates.forEach(({ t }, index) => {
        ...
        let score = eligible
            ? t.score === undefined
                ? 1
                : finite(typeof t.score === "function" ? t.score(snapshot.board, event) : t.score, "InvalidDeusTransition", `transition ${t.key} score`)
            : 0;
        if (eligible && t.utility) {
            utility = judgeUtility({ board: snapshot.board, event }, t.utility.map((u) => ({
                key: u.key,
                when: (ctx) => u.when?.(ctx.board, ctx.event) ?? true,
                score: (ctx) => (typeof u.score === "function" ? u.score(ctx.board, ctx.event) : u.score),
                reason: typeof u.reason === "function"
                    ? (ctx) => {
            ...
            if (!utility.selected)
                ...
                if (t.score === undefined)
                    score = utility.selected.score;
            }
        }
        const to = eligible && t.to
            ? resolveTransitionTargetPath(typeof t.to === "function" ? t.to(snapshot.board, event) : t.to, t.from, `transition ${t.key} to`)
            : undefined;
        if (to) {
...
export function formatDeusStepTrace(trace) {
