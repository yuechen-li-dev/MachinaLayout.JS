...
const validateAllowedSuffixes = (
  ...
  if (!Array.isArray(allowedSuffixes) || allowedSuffixes.length !== expectedLength) {
    throw new MachinaDispatchError("InvalidDispatchTable", `${tableName}.allowedSuffixes length mismatch`);
  }
  for (const row of allowedSuffixes) {
    if (row !== undefined && !isStringArray(row)) {
      throw new MachinaDispatchError("InvalidDispatchTable", `${tableName}.allowedSuffixes must be string arrays`);
    }
  }
};

const validateBy = (by: readonly number[] | undefined, expectedLength: number, tableName: string) => {
  ...
  for (const value of by) {
    if (typeof value !== "number" || !Number.isFinite(value)) {
      throw new MachinaDispatchError("InvalidDispatchTable", `${tableName}.by must contain finite numbers`);
    }
  }
};
...
export const validateSetTable = <TState>(table: SetDispatchTable<TState>): void => {
  ...
  if (!isStringArray(table.events)) throw new MachinaDispatchError("InvalidDispatchTable", "set.events must be strings");
};
export const validateToggleTable = <TState>(table: ToggleDispatchTable<TState>): void => {
  ...
  if (!isStringArray(table.events)) throw new MachinaDispatchError("InvalidDispatchTable", "toggle.events must be strings");
};
export const validateIncrementTable = <TState>(table: IncrementDispatchTable<TState>): void => {
  ...
  if (!isStringArray(table.events)) throw new MachinaDispatchError("InvalidDispatchTable", "increment.events must be strings");
...
export const validatePrefixSetTable = <TState>(table: PrefixSetDispatchTable<TState>): void => {
  ...
  if (!isStringArray(table.prefixes)) throw new MachinaDispatchError("InvalidDispatchTable", "setSuffix.prefixes must be strings");
  validateAllowedSuffixes(table.allowedSuffixes, table.prefixes.length, "setSuffix");
};
export const validatePrefixIncrementTable = <TState>(table: PrefixIncrementDispatchTable<TState>): void => {
  validateLengths("incrementSuffix", table.prefixes, table.fields as readonly unknown[]);
  if (!isStringArray(table.prefixes)) throw new MachinaDispatchError("InvalidDispatchTable", "incrementSuffix.prefixes must be strings");
...
export function defineDispatchTables<TState>(tables: MachinaDispatchTables<TState>): MachinaDispatchTables<TState> {
...
export function resolveEventValue<TValue>(
  ...
  if (!isStringArray(table.events)) throw new MachinaDispatchError("InvalidDispatchTable", "events must be strings");
...
export function matchEventPrefix(event: string, prefix: string, allowedSuffixes?: readonly string[]): string | undefined {
  ...
  if (allowedSuffixes !== undefined && !isStringArray(allowedSuffixes)) {
    throw new MachinaDispatchError("InvalidDispatchTable", "allowedSuffixes must be a string array");
  }
