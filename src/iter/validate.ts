import type { IterDiagnostic, IterMachine } from "./types";

function pushDiagnostic(
  diagnostics: IterDiagnostic[],
  severity: IterDiagnostic["severity"],
  code: string,
  message: string,
  path?: string,
): void {
...
export function validateIterMachine<TEnv, TCursor, TYield, TReturn, TError>(
  machine: IterMachine<TEnv, TCursor, TYield, TReturn, TError>,
): IterDiagnostic[] {
  const diagnostics: IterDiagnostic[] = [];

  if (machine.kind !== "iterMachine") {
    pushDiagnostic(
      diagnostics,
      "error",
      "InvalidIterMachineKind",
      'Iter machine kind must be "iterMachine".',
      "kind",
    );
  }
  ...
  if (typeof machine.id !== "string" || machine.id.trim().length === 0) {
    pushDiagnostic(
      ...
      "Iter machine id must be a non-empty string.",
      "id",
    );
  }
  ...
  if (typeof machine.step !== "function") {
    pushDiagnostic(
      ...
      "Iter machine step must be a function.",
      "step",
    );
  }
  ...
  if (machine.description !== undefined && typeof machine.description !== "string") {
    pushDiagnostic(
      ...
      "Iter machine description must be a string when provided.",
      "description",
    );
  }
...
export function formatIterDiagnostics(diagnostics: readonly IterDiagnostic[]): string {
