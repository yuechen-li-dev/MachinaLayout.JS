...
function pushDiagnostic(
  diagnostics: ConceptDiagnostic[],
  severity: ConceptDiagnostic["severity"],
  code: string,
  message: string,
  path?: string,
): void {
...
function isSupportedLiteralValue(value: unknown): value is string | number | boolean {
  return typeof value === "string" || typeof value === "number" || typeof value === "boolean";
}

function areConstraintsEquivalent(
  left: ConceptFieldConstraint,
  right: ConceptFieldConstraint,
): boolean {
...
function formatConstraint(constraint: ConceptFieldConstraint): string {
...
function validateConstraint(
  constraint: ConceptFieldConstraint,
  path: string,
  diagnostics: ConceptDiagnostic[],
): void {
  if (!CONSTRAINT_KINDS.has(constraint.kind)) {
    pushDiagnostic(
      diagnostics,
      "error",
      "InvalidConceptConstraint",
      `Constraint kind '${String(constraint.kind)}' is not supported.`,
      path,
    );
    return;
  }
  ...
  if (constraint.kind === "literal" && !isSupportedLiteralValue(constraint.value)) {
    pushDiagnostic(
      ...
      "error",
      "InvalidConceptConstraint",
      "Literal constraints must use string, number, or boolean values.",
      path,
    );
  }
}

function validateConceptDefinitionInner(
  concept: ConceptDefinition,
  diagnostics: ConceptDiagnostic[],
  visited: Set<ConceptDefinition>,
): void {
  if (visited.has(concept)) {
    pushDiagnostic(
      ...
      "error",
      "InvalidConceptConstraint",
      `Recursive concept composition is not supported for '${concept.id}'.`,
      "composedFrom",
    );
    return;
  }
  ...
  if (
    ...
    pushDiagnostic(
      ...
      "error",
      "InvalidConceptConstraint",
      "Concept fields must be a record of field constraints.",
      "fields",
    );
  } else {
    for (const [fieldName, constraint] of Object.entries(concept.fields)) {
      if (fieldName.trim().length === 0 || !isSafeFieldName(fieldName)) {
        pushDiagnostic(
          ...
          "error",
          "InvalidConceptField",
          `Concept field '${fieldName}' is empty or not safe-ish.`,
          `fields.${fieldName}`,
        );
      }
  ...
  const seenFields = new Map<string, { constraint: ConceptFieldConstraint; source: string }>();
    ...
    for (const [fieldName, constraint] of Object.entries(sourceConcept.fields)) {
      ...
      if (seen && !areConstraintsEquivalent(seen.constraint, constraint)) {
        pushDiagnostic(
          diagnostics,
          "error",
          "ConflictingConceptField",
          `Field '${fieldName}' conflicts between composed concepts '${seen.source}' and '${sourceConcept.id}' (${formatConstraint(seen.constraint)} vs ${formatConstraint(constraint)}).`,
          `fields.${fieldName}`,
        );
      } else if (!seen) {
...
function isConstraintSatisfied(constraint: ConceptFieldConstraint, value: unknown): boolean {
...
export function validateConceptDefinition(concept: ConceptDefinition): ConceptDiagnostic[] {
  const diagnostics: ConceptDiagnostic[] = [];
  validateConceptDefinitionInner(concept, diagnostics, new Set<ConceptDefinition>());
  return diagnostics;
}

export function validateConceptValue(
  concept: ConceptDefinition,
  value: unknown,
): ConceptDiagnostic[] {
    ...
    if (!record || !hasField(record, fieldName)) {
      if (!constraint.optional) {
        pushDiagnostic(
          ...
          "error",
          "MissingConceptField",
          `Required concept field '${fieldName}' is missing.`,
          fieldName,
        );
      }
      continue;
    ...
    if (!isConstraintSatisfied(constraint, fieldValue)) {
      if (constraint.kind === "literal") {
        pushDiagnostic(
          ...
          `Field '${fieldName}' must equal ${JSON.stringify(constraint.value)}.`,
          fieldName,
        );
      } else {
        pushDiagnostic(
          ...
          `Field '${fieldName}' must be ${constraint.kind}.`,
          fieldName,
        );
      }
    }
...
export function assertConceptValue(concept: ConceptDefinition, value: unknown): void {
...
export function formatConceptDiagnostics(diagnostics: readonly ConceptDiagnostic[]): string {
