...
describe("MachinaVueTextView", () => {
  ...
  it("plain source remains literal", () => {
    const w = mount(MachinaVueTextView, { props: { text: { kind: "plain", text: "Hello **not bold**" } } });
    expect(w.text()).toContain("Hello **not bold**");
    expect(w.find("strong").exists()).toBe(false);
  });
  it("policy variant leading align valign", () => {
    const w = mount(MachinaVueTextView, { props: { text: { kind: "text", source: { kind: "machina-text", text: "Title" }, variant: "title", leading: "tight", align: "center", valign: "center" } } });
  ...
  it("overflow ellipsis overrides wrap", () => {
    const w = mount(MachinaVueTextView, { props: { text: { kind: "text", source: { kind: "machina-text", text: "Long" }, wrap: "word", overflow: "ellipsis" } } });
    const style = w.attributes("style");
  ...
  it("wrap word + clip remains wrapping", () => {
    const w = mount(MachinaVueTextView, { props: { text: { kind: "text", source: { kind: "machina-text", text: "Long" }, wrap: "word", overflow: "clip" } } });
    const style = w.attributes("style");
  ...
  it("renders link and click callback", async () => {
    ...
    const w = mount(MachinaVueTextView, { props: { text: "Read [docs](https://example.com)", onLinkClick } });
    const a = w.find("a");
  ...
  it("link target blank sets rel", () => {
    const w = mount(MachinaVueTextView, { props: { text: "Read [docs](https://example.com)", linkTarget: "_blank" } });
    const a = w.find("a");
  ...
  it("renders bullet list with nesting", () => {
    const w = mount(MachinaVueTextView, { props: { text: "- Build rows\n- Resolve rectangles\n  - Preserve order\n- Render views" } });
    expect(w.findAll("ul").length).toBeGreaterThan(1);
    expect(w.text()).toContain("Preserve order");
  });
  it("applies blockGap and listGap", () => {
    const w = mount(MachinaVueTextView, { props: { text: { kind: "text", source: { kind: "machina-text", text: "First\n\nSecond\n\n- A\n- B" }, blockGap: 12, listGap: 6 } } });
  ...
  it("fills parent and hook styles/classes", () => {
    const w = mount(MachinaVueTextView, { props: { text: "Use `x` and [a](https://e.com)", rootClass: "root-x" as any, rootStyle: { color: "red" }, linkClass: "lnk" as any, linkStyle: { color: "blue" }, codeClass: "cd" as any, codeStyle: { color: "green" } } });
