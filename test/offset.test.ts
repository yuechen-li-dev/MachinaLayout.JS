...
describe("applyOffset", () => {
  it("applies numeric offsets", () => {
    ...
    expect(applyOffset(rect, parent, { x: 10, y: -5 })).toEqual({
      x: 130,
      y: 235,
      width: 50,
      height: 30,
  ...
  it("supports px and ui objects and does not mutate input", () => {
    ...
    const rect = { x: 120, y: 240, width: 50, height: 30 };
    const offset = {
      x: { unit: "ui", value: 0.1 } as const,
      y: { unit: "ui", value: 0.1 } as const,
    };
    ...
    expect(
      applyOffset(rect, parent, { x: { unit: "px", value: 8 }, y: { unit: "px", value: -4 } }),
    ).toEqual({ x: 128, y: 236, width: 50, height: 30 });
