...
describe("selectLayoutRowsForRoot", () => {
    ...
    const rows: LayoutRow[] = [
      { id: "root", frame: { kind: "root" } },
      {
        ...
        variants: [
          {
            when: { maxWidth: 200 },
            frame: { kind: "absolute", x: 1, y: 1, width: 10, height: 10 },
          },
        ],
      },
    ];
    ...
    const rows: LayoutRow[] = [
      { id: "root", frame: { kind: "root" } },
      {
        ...
        variants: [
          ...
          {
            when: { maxWidth: 1000 },
            frame: { kind: "absolute", x: 2, y: 2, width: 20, height: 20 },
          },
        ],
      },
    ];
  ...
  it("supports inclusive width and height bounds and empty fallback", () => {
    const rows: LayoutRow[] = [
      { id: "root", frame: { kind: "root" } },
      {
        ...
        variants: [
          {
            when: { minWidth: 500, maxWidth: 800, minHeight: 300, maxHeight: 600 },
            frame: { kind: "absolute", x: 5, y: 5, width: 50, height: 50 },
          },
    ...
    expect(selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: 500, height: 300 })[1].frame).toEqual(
      { kind: "absolute", x: 5, y: 5, width: 50, height: 50 },
    );
    expect(selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: 800, height: 600 })[1].frame).toEqual(
      { kind: "absolute", x: 5, y: 5, width: 50, height: 50 },
    );
    expect(selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: 499, height: 500 })[1].frame).toEqual(
      { kind: "absolute", x: 9, y: 9, width: 90, height: 90 },
    );
    expect(selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: 801, height: 500 })[1].frame).toEqual(
      { kind: "absolute", x: 9, y: 9, width: 90, height: 90 },
    );
  });
  ...
  it("validates variant conditions and z", () => {
    ...
    expect(() =>
      selectLayoutRowsForRoot(mk({ when: { maxWidth: Number.NaN } }), rootRect),
    ).toThrowError(expect.objectContaining({ code: "NonFiniteNumber" }));
    expect(() =>
      selectLayoutRowsForRoot(mk({ when: { minHeight: Number.POSITIVE_INFINITY } }), rootRect),
    ).toThrowError(expect.objectContaining({ code: "NonFiniteNumber" }));
    expect(() =>
      selectLayoutRowsForRoot(mk({ when: { minWidth: 900, maxWidth: 800 } }), rootRect),
    ).toThrowError(expect.objectContaining({ code: "InvalidVariantCondition" }));
    expect(() =>
      selectLayoutRowsForRoot(mk({ when: { minHeight: 700, maxHeight: 600 } }), rootRect),
    ).toThrowError(expect.objectContaining({ code: "InvalidVariantCondition" }));

    expect(() => selectLayoutRowsForRoot(mk({ when: {}, z: 6 }), rootRect)).toThrowError(
      expect.objectContaining({ code: "InvalidZ" }),
    );
    expect(() => selectLayoutRowsForRoot(mk({ when: {}, z: 1.5 }), rootRect)).toThrowError(
      expect.objectContaining({ code: "InvalidZ" }),
    );
    expect(() => selectLayoutRowsForRoot(mk({ when: {}, z: Number.NaN }), rootRect)).toThrowError(
  ...
  it("validates rootRect", () => {
    ...
    expect(() => selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: -1, height: 10 })).toThrowError(
      expect.objectContaining({ code: "NegativeSize" }),
    );
    expect(() => selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: 10, height: -1 })).toThrowError(
      expect.objectContaining({ code: "NegativeSize" }),
    );
    expect(() =>
      selectLayoutRowsForRoot(rows, { x: Number.NaN, y: 0, width: 10, height: 10 }),
    ).toThrowError(expect.objectContaining({ code: "NonFiniteNumber" }));
  });
...
describe("responsive variants integration", () => {
  it("resolveLayoutRows applies variants before compile", () => {
    const rows: LayoutRow[] = [
      { id: "root", frame: { kind: "root" } },
      {
        id: "child",
        parent: "root",
        frame: { kind: "absolute", x: 10, y: 10, width: 300, height: 100 },
        variants: [
          {
            when: { maxWidth: 600 },
  ...
  it("compileLayoutRows direct path ignores variants", () => {
    const rows: LayoutRow[] = [
      { id: "root", frame: { kind: "root" } },
      {
        ...
        variants: [
          {
            when: {},
            frame: { kind: "absolute", x: 10, y: 20, width: 30, height: 40 },
            view: "variant",
          },
        ],
      },
    ...
    expect(document.nodes.child.frame).toEqual({
      kind: "absolute",
      x: 1,
      y: 2,
      width: 3,
    ...
    const rows: LayoutRow[] = [
      {
        id: "root",
        frame: { kind: "root" },
        arrange: { kind: "stack", axis: "horizontal", gap: 0 },
      },
      { id: "left", parent: "root", order: 0, frame: { kind: "fixed", width: 100, height: 100 } },
      {
        ...
        variants: [{ when: { maxWidth: 500 }, frame: { kind: "fill", weight: 1 } }],
      },
    ];
...
it("variant can override layer", () => {
  const rows: LayoutRow[] = [
    { id: "root", frame: { kind: "root" } },
    {
      ...
      frame: { kind: "absolute", x: 0, y: 0, width: 10, height: 10 },
      layer: "base",
      variants: [{ when: { maxWidth: 600 }, layer: "overlay" }],
    },
  ];
  expect(
    selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: 500, height: 500 }).find(
      (r) => r.id === "child",
    )?.layer,
  ).toBe("overlay");
  expect(
    selectLayoutRowsForRoot(rows, { x: 0, y: 0, width: 800, height: 500 }).find(
      (r) => r.id === "child",
    )?.layer,
  ).toBe("base");
});
