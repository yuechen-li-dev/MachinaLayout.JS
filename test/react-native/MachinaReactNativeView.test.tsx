...
describe("MachinaReactNativeView", () => {
  ...
  it("applies numeric absolute styles and nested normalization", async () => {
    ...
    expect(main.props.style[0]).toMatchObject({
      position: "absolute",
      left: 268,
      top: 88,
      width: 816,
  ...
  it("supports view/slot, view precedence, data, z sorting, debug and no mutation", async () => {
    ...
    const Panel = (props: any) => {
      seen.push(props);
      return React.createElement("rn-panel", { id: props.id });
    };
    ...
    await act(async () => {
      tree = renderer.create(
        <MachinaReactNativeView
          ...
          debug
          views={{ Panel, Slot }}
          viewData={{ Panel: { value: 1 }, Slot: { value: 2 } }}
          nodeData={{ panel: { p: true } }}
          layers={{ base: { z: 0 }, overlay: { z: 3 } }}
        />,
      );
    });
