...
function stackDoc(
  ...
  return {
    ...
    nodes: {
      ...
      parent: {
        id: "parent",
        frame: { kind: "absolute", x: 0, y: 0, width: parentSize.w, height: parentSize.h },
        arrange: { kind: "stack", axis },
      },
      ...children,
    },
...
describe("stack arrange fill", () => {
  ...
  it("equal and weighted fill sizes", () => {
    const eq = stackDoc(
      ...
      { w: 300, h: 20 },
      {
        a: { id: "a", frame: { kind: "fill", cross: 20 } },
        b: { id: "b", frame: { kind: "fill", cross: 20 } },
        c: { id: "c", frame: { kind: "fill", cross: 20 } },
      },
    );
    let r = resolveLayoutDocument(eq, { x: 0, y: 0, width: 500, height: 100 });
    expect(r.nodes.a.rect.width).toBe(100);
    expect(r.nodes.b.rect.width).toBe(100);
    expect(r.nodes.c.rect.width).toBe(100);
    const wt = stackDoc(
      "horizontal",
      { w: 300, h: 20 },
      {
        a: { id: "a", frame: { kind: "fill", weight: 1, cross: 20 } },
        b: { id: "b", frame: { kind: "fill", weight: 2, cross: 20 } },
      },
    );
    r = resolveLayoutDocument(wt, { x: 0, y: 0, width: 500, height: 100 });
    expect(r.nodes.a.rect.width).toBe(100);
    expect(r.nodes.b.rect.width).toBe(200);
  });
      ...
      () =>
        resolveLayoutDocument(
          stackDoc(
            "horizontal",
            { w: 100, h: 20 },
            { f: { id: "f", frame: { kind: "fill", weight: 0 } } },
          ),
      ...
      () =>
        resolveLayoutDocument(
          stackDoc(
            "horizontal",
            { w: 100, h: 20 },
            { f: { id: "f", frame: { kind: "fill", weight: Number.NaN } } },
          ),
          { x: 0, y: 0, width: 1, height: 1 },
        ),
      "NonFiniteNumber",
      ...
      () =>
        resolveLayoutDocument(
          stackDoc(
            "horizontal",
            { w: 100, h: 20 },
            { f: { id: "f", frame: { kind: "fill", cross: -1 } } },
          ),
          { x: 0, y: 0, width: 1, height: 1 },
        ),
      "NegativeSize",
      ...
      () =>
        resolveLayoutDocument(
          stackDoc(
            "horizontal",
            { w: 100, h: 20 },
            { f: { id: "f", frame: { kind: "fill", cross: 30 } } },
          ),
          { x: 0, y: 0, width: 1, height: 1 },
        ),
      "StackOverflow",
...
it("offset on fixed child does not affect siblings", () => {
  ...
  expect(r.nodes.a.rect.x).toBe(0);
  expect(r.nodes.b.rect.x).toBe(120);
  expect(r.nodes.c.rect.x).toBe(200);
});
