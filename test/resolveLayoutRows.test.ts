  ...
  it("compiles rows and resolves rectangles", () => {
    const rows: LayoutRow[] = [
      { id: "root", frame: { kind: "root" } },
      {
        id: "child",
        parent: "root",
        frame: { kind: "absolute", x: 10, y: 20, width: 300, height: 40 },
      },
    ];
...
it("rejects FillFrame under non-arranging parent", () => {
  ...
  expect(() => resolveLayoutRows(rows, { x: 0, y: 0, width: 100, height: 100 })).toThrowError(
    MachinaLayoutError,
  );
  try {
    resolveLayoutRows(rows, { x: 0, y: 0, width: 100, height: 100 });
  } catch (e) {
